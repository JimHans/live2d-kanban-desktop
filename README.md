...
- 全新的接入原生通知的日程提醒功能，只要输入提醒时间与提醒内容即可到点提醒，同时包括多种取自MIUI的随机通知铃声，亦可自定义通知音效
...
- V2与V3+全版本模型自定义缩放与模型坐标位置，自由调节模型显示大小与位置
...
---
## 📺安装：
请移步到本项目Release中，获取最新版本安装包。稳定版Release中默认包含Windows平台下zip绿色版与安装程序，同时主要版本均同时包含在Endeavour OS环境下编译的Linux版本可执行程序。由于设备限制，Mac用户需要自行搭建环境进行编译使用，感谢理解。

*如果你正在使用Arch Linux，你也可以通过[AUR软件仓库](https://aur.archlinux.org/packages/live2d-kanban-desktop)安装本软件（感谢[@zxp19821005](https://github.com/zxp19821005)）*
<br/><br/>测试版Prerelease通道将发布提前预览的版本，拥有更新的功能的同时由于未开发完成，可能拥有影响使用的Bug，仅供尝鲜测试。
