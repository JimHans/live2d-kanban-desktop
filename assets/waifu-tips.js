window.live2d_settings = Array(); /*
    ...
    　　 　 /　 ／,　 /|　 ,　 ,　　　 ',     ||||| |||        Version 1.0.0_425_Desktop (Based on V1.0.7)
    　　　ｲ 　/ /-‐/　ｉ　L_ ﾊ ヽ!　 i      ||   | ||||       Update 2021-05-19 Modified By JimHan From the FGHRSH Version
    　　　 ﾚ ﾍ 7ｲ｀ﾄ　 ﾚ'ｧ-ﾄ､!ハ|　 |
    　　　　 !,/7 '0'　　 ´0iソ| 　 |
    　　　　 |.从"　　_　　 ,,,, / |./ 　 |                     网页添加 Live2D 看板娘
...
// 其他杂项设置
live2d_settings.l2dVersion           = '1.0.0_425_Desktop';                   // 当前版本
live2d_settings.l2dVerDate           = '2021.04.25';                          // 版本更新日期
live2d_settings.homePageUrl          = '';                                    // 主页地址，无需填写（已弃用）
...
/*************************************亮度遮罩*************************************/
	...
	{
		showMessagePRO('设备使用时长提醒已开启！设定时间为'+setTime+'分钟。再次单击关闭计时！',5000);
		timer = setInterval(function(){showMessage('您已使用'+setTime+'分钟！休息一下，放松放松吧！',5000,true);
		var rand = Math.floor(Math.random()*6);
		var audio= new Audio("./Alert Alarms/alert"+rand+".mp3");//这里的路径写上mp3文件在项目中的绝对路径
        audio.play();//播放
		showMessage('您已使用'+setTime+'分钟！休息一下，放松放松吧！',5000,true);date = new Date();},setTime*60000);
	}
/***********************************独立的通知提醒框****************************************/
	...
	'use strict';
        ...
        $('.waifu-tips-PRO').stop();
        $('.waifu-tips-PRO').html(text).fadeTo(200, 1);
        if (timeout !== undefined)
        {$('.waifu-tips-PRO').stop().css('opacity',1);
        window.setTimeout(function() {sessionStorage.removeItem('waifu-text');}, timeout);
